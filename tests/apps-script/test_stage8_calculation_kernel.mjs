...
test('canonical Calculation Result schema is Draft 2020-12 valid', () => {
  const schema = JSON.parse(readFileSync(resolve(root, 'contracts/legacy-apps-script/calculation-result.schema.json'), 'utf8'));
...
test('project-input-v2 live regression keeps dishwasher and sink distinct and excludes countertop', () => {
  const fixture = JSON.parse(readFileSync(resolve(root, 'fixtures/legacy/stage9-live-project-input-v2.json'), 'utf8'));
  const schema = JSON.parse(readFileSync(resolve(root, 'contracts/legacy-apps-script/project-input.schema.json'), 'utf8'));
...
test('Apps Script layout matches shared Python Stage 3 golden fixtures and is deterministic', () => {
  const fixture = JSON.parse(readFileSync(resolve(root, 'fixtures/legacy/stage8-layout-golden.json'), 'utf8'));
