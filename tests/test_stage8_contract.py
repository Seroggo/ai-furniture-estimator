...
class Stage8ContractTests(unittest.TestCase):
    def test_calculation_result_schema_is_canonical_draft_2020_12(self) -> None:
        schema = json.loads(
            (ROOT / "docs/stage-8-calculation-kernel/calculation-result.schema.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_shared_layout_golden_is_explicitly_python_reference_generated(self) -> None:
        fixture = json.loads(
            (ROOT / "tests/fixtures/stage8-layout-golden.json").read_text(encoding="utf-8")
        )
        self.assertEqual("calculation_model/layout_configurator.py", fixture["reference"])
        self.assertEqual(
            {
